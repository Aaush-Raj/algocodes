...
print("Bank Nifty 930 100 OTM started for Nitin Hirani. Time stamp: ",dt.datetime.now())
...
def main():
	...
	while dt.datetime.now().time() <= dt.time(9,29,56):
		sleep(2)
	...
	try:
		...
		while sl_to_cost == False:
			...
			if call_order_status and put_order_status == 'rejected':
...
def call_slm_order_placement():
	...
	if call_order_status == 'complete':
		slm_call_buy_price = 0.05 * round((float(call_sell_avg_price) + 50)/0.05)
...
def put_slm_order_placement():
	...
	if put_order_status == 'complete':
		slm_put_buy_price = 0.05 * round((float(put_sell_avg_price) + 50)/0.05)
