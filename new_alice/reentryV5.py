...
from common_functionsV2 import generate_session, check_margin, order_status, get_oid, ap_generator, latest_expiry,monthly_expiry, modify_ce_order_to_cost,modify_pe_order_to_cost, get_ltp_info,strLightPurple,strGreen,strRed,strBold
...
if get_closest_val == True:
    if closest_type == "NEAR":
        ce_stk = closest_value(kite.ltp(instrument_ce),closest_target_ce)
        pe_stk = closest_value(kite.ltp(instrument_pe),closest_target_pe)

    if closest_type == "ABOVE":
        ce_stk = above_closest_value(kite.ltp(instrument_ce),closest_target_ce)
        pe_stk = above_closest_value(kite.ltp(instrument_pe),closest_target_pe)

    if closest_type == "BELOW":
        ce_stk = below_closest_value(kite.ltp(instrument_ce),closest_target_ce)
        pe_stk = below_closest_value(kite.ltp(instrument_pe),closest_target_pe)
