...
def main():
	...
	try:
		...
		while order_placed == False:
			...
			print(curr_ltp)
			ce,pe = int(round(curr_ltp/50)*50),int(round(curr_ltp/50)*50)
		...
		while dt.datetime.now().time() < dt.time(15,9,59):
            ...
            if order_status_function(oid_call_slm) == "complete":
                ...
                curr_ltp = get_ltp_info(alice.get_instrument_by_symbol('INDICES',symbol))
                print(curr_ltp)
                ce= int(round(curr_ltp/50)*50)
                print('ATM CE ',ce)
                get_ce_curr_price(ce)
                call_slm_order_placement()
            ...
            if order_status_function(oid_call_slm) == "complete":
                ...
                curr_ltp = get_ltp_info(alice.get_instrument_by_symbol('INDICES',symbol))
                print(curr_ltp)
                pe= int(round(curr_ltp/50)*50)
                print('ATM CE ',pe)
                get_ce_curr_price(pe)
                call_slm_order_placement()
                get_pe_curr_price(pe)
                put_slm_order_placement()
...
def call_hedge_placement():
	...
	bnf_hedge_call = alice.get_instrument_for_fno(exch = 'NFO',symbol='NIFTY',expiry_date=datecalc,is_fut=False,strike=bnf_call_hedge,is_CE=True)
...
def put_hegde_placement():
	...
	bnf_hedge_put = alice.get_instrument_for_fno(exch = 'NFO',symbol='NIFTY',expiry_date=datecalc,is_fut=False,strike=bnf_put_hedge,is_CE=False)
...
modify_order = False
symbol = 'NIFTY BANK'
no_of_lots = us_HUF028.no_of_lots_four_lot * 4
