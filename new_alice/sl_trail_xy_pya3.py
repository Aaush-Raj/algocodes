...
def main():
    ...
    CE_SL_PRC = tgp_bn_ce_sell
    PE_SL_PRC = tgp_bn_pe_sell
    ...
    while dt.datetime.now().time() < dt.time(14,59,59):
        ...
        if int(ce_avg_p - LTP_CE) >=trail_trigger:
            print("TIME TO MODIFY SLM CE ORDER TO 20 POINTS UP")
            CE_SL_PRC = CE_SL_PRC - trail
            mod_ce = alice.modify_order(transaction_type = TransactionType.Buy,
                        ...
                        order_id=sl_ce_oid,
                                    ...
                                    product_type = ProductType.Intraday,
                                    price = CE_SL_PRC + 10,
                                    trigger_price = CE_SL_PRC)
            ce_avg_p -= 50
            
        if int(LTP_PE - pe_avg_p) >=trail_trigger:
