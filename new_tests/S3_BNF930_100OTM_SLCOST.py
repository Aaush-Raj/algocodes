...
client_code = us_MA012.client_code
full_name = us_MA012.full_name
username = us_MA012.username
password = us_MA012.password
twoFA = us_MA012.twoFA
api_key = us_MA012.api_key
user_gmail_id = us_MA012.user_gmail_id
my_gmail_id = 'hiraninitin96@gmail.com'
strategy = "S3 Four lot"
...
def main():
	...
	try:
		...
		while dt.datetime.now().time() <= dt.time(9,29,58):
			sleep(2)
		...
		while order_placed == False:
			...
			print("PUT SLM PLACEMENT TRIGGERED")
			order_placed = True
		...
		while sl_to_cost == False:

			print("second loop is running bro")

			if order_status_function(oid_call_slm) == 'complete':
				modify_pe_order_to_cost(oid_put_slm)
				sl_to_cost = True
			elif order_status_function(oid_put_slm) == 'complete':
				modify_ce_order_to_cost(oid_call_slm)
				sl_to_cost = True
			else:
				if dt.datetime.now().time() > dt.time(14,59):
					sl_to_cost = True
				else:
					sl_to_cost = False
			sleep(10)
		...
		subject = "Error in cliend id:","user id:",client_code," ",username," ",strategy
...
def call_slm_order_placement():
	...
	if call_order_status == 'complete':
		slm_call_buy_price = 0.05 * round((float(call_sell_avg_price) * 1.80)/0.05)
...
def put_slm_order_placement():
	...
	if put_order_status == 'complete':
		slm_put_buy_price = 0.05 * round((float(put_sell_avg_price) * 1.80)/0.05)
...
modify_order = False
symbol = 'NIFTY BANK'
no_of_lots = us_MA012.no_of_lots_four_lot
