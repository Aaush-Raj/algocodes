...
def sell_atm_options():
    global bn_ce_sell_order_price,bn_pe_sell_order_price
...
order_placed = False
while datetime.datetime.now().time() < datetime.time(9,45):
    print("wait for the time to be 9:45 ; Right now the time is only -->)",datetime.datetime.now().time())
    time.sleep(5)
try:
    sell_atm_options()
    stoploss_order()
